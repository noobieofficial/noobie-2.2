...
def parse_command_with_variables(line: str, variables: Dict[str, Variable]) -> str:
    """
    Parse command line replacing variables and concatenating string parts.
    This function handles variable substitution outside of quotes.
    """
    # First, replace variables with their values
    processed_line = replace_variables(line, variables)
    
    # Split the line into parts, preserving quoted strings
    parts = []
    current_part = ""
    in_quotes = False
    quote_char = None
...
def concatenate_message_parts(parts: list, start_index: int = 1) -> str:
    """
    Concatenate message parts for commands like SAY and HEAR.
    This handles the concatenation of quoted strings and variable values.
    """
    if len(parts) <= start_index:
        return ""
    
    message_parts = parts[start_index:]
    result = ""
    
    for part in message_parts:
        # Remove quotes from string literals but keep the content
        if part.startswith('"') and part.endswith('"'):
            result += part[1:-1]  # Remove surrounding quotes
        elif part.startswith("'") and part.endswith("'"):
            result += part[1:-1]  # Remove surrounding quotes
        else:
            # This is a variable value or unquoted text
            result += str(part)
